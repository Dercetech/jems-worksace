declare global {
  namespace Phaser.Scene.Systems {
...
export class AnimatedTiles extends Phaser.Plugins.ScenePlugin {
  ...
  public static ID = "AnimatedTiles";
  ...
  boot() {
    if (!this.scene.sys.settings.isBooted) {
      ...
      this.scene.sys.settings.isBooted = true;
    }
  }
  ...
  init(map: Phaser.Tilemaps.Tilemap) {
    // console.log(">>>> init");
    // console.log(">>>>>>>>> count " + this.animatedTiles.length);
    // console.log(">>>>>>>>> activeLayer " + this.activeLayer.length);
  ...
  shutdown() {
    // console.log("should do some cleanup for memory leaks here");
  ...
  getAnimatedTiles(map) {
    ...
    map.tilesets.forEach(
      ...
      (tileset) => {
        ...
        Object.keys(tileData).forEach((indexAsString) => {
    ...
    container.forEach((mapAnimData) => {
      ...
      mapAnimData.animatedTiles.forEach((tileAnimData) => {
        tileAnimData.tiles.forEach((tiles, layerIndex) => {
          ...
          if (!layer || layer.type === "StaticTilemapLayer") {
            return;
          }
